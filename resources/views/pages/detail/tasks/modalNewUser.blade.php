...
@section('content')
    <div class="absolute inset-0 z-1 bg-[rgba(238,238,238,0.5)] backdrop-blur-[10px] flex justify-center items-center">
        <div
            ...
            <a href="{{ route('tasks.anotherTask', ['id' => $task->id]) }}"
                class="text-[16px] border py-[4px] px-[8px] rounded-[8px] w-full bg-[#3D90D7] text-[#eeeeee] font-bold hover:bg-[#3A59D1] cursor-pointer">Copy From Another Task</a>
            <form action="{{ route('backend.tasks.users.store', ['id' => $task->id]) }}" method="POST"
                class="flex flex-col gap-[16px] w-full">
                @csrf
                <label for="user_id">Pilih User:</label>
                <select name="user_id[]" id="user_id" multiple class="w-full">
                ...
                <button
                    class="text-[16px] border py-[4px] px-[8px] rounded-[8px] w-full bg-[#3D90D7] text-[#eeeeee] font-bold hover:bg-[#3A59D1] cursor-pointer">
                    Submit
                </button>
            </form>
        </div>
