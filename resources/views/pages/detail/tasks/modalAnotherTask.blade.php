...
@section('title', 'Modal Another Task')
...
@section('content')
    <div class="absolute inset-0 z-1 bg-[rgba(238,238,238,0.5)] backdrop-blur-[10px] flex justify-center items-center">
        <div
            ...
            <h1 class="text-[32px] font-bold">Choose Task</h1>
            <form action="{{ route('tasks.newUser', ['id' => $task->id]) }}" method="GET"
                class="flex justify-center gap-[16px] w-full">
                @csrf
                <input type="text" name="search" class="border" class="border">
                <button type="submit" class="py-[4px] px-[16px] border">Search</button>
            </form>
            <form action="{{ route('backend.tasks.users.copy', ['id' => $task->id]) }}" method="POST"
                class="flex flex-col gap-[16px] w-full">
                @csrf
                <select name="task_id" class="border py-[4px] px-[16px] rounded-[8px]">
